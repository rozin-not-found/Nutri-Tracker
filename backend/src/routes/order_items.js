...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    await Order_itemsService.create(
      req.body.data,
      req.currentUser,
      true,
      link.host,
...
router.put(
  '/:id',
  wrapAsync(async (req, res) => {
    await Order_itemsService.update(
      req.body.data,
      req.body.id,
      req.currentUser,
    );
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await Order_itemsDBApi.findAll(req.query, globalAccess);
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'quantity', 'total_price'];
      const opts = { fields };
      try {
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await Order_itemsDBApi.findBy({ id: req.params.id });
