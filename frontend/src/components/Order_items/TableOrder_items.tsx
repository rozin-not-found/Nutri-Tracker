...
const TableSampleOrder_items = ({
  ...
  const {
    ...
    loading,
    count,
    notify: order_itemsNotify,
    refetch,
  } = useAppSelector((state) => state.order_items);
  ...
  useEffect(() => {
    if (order_itemsNotify.showNotification) {
      notify(
        order_itemsNotify.typeNotification,
        order_itemsNotify.textNotification,
      );
    }
  ...
  const handleEditAction = (id: string) => {
    router.push(`/order_items/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/order_items/order_items-view/?id=${id}`);
  };
