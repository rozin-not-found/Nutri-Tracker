...
import { loadColumns } from './configureShopCols';
...
const TableSampleShop = ({
  ...
  useEffect(() => {
    if (shopNotify.showNotification) {
      notify(shopNotify.typeNotification, shopNotify.textNotification);
    }
  }, [shopNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/shop/shop-view/?id=${id}`);
  };
...
export default TableSampleShop;
