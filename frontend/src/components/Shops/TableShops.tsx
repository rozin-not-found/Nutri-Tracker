...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/shops/shopsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureShopsCols';
...
const TableSampleShops = ({
  ...
  useEffect(() => {
    if (shopsNotify.showNotification) {
      notify(shopsNotify.typeNotification, shopsNotify.textNotification);
    }
  }, [shopsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/shops/shops-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {shops && Array.isArray(shops) && !showGrid && (
        <ListShops
          shops={shops}
          loading={loading}
...
export default TableSampleShops;
